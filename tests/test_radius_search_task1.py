...
class TestStructureLocationsFile:
    ...
    def test_structure_locations_has_required_keys(self):
        """structure_locations.json should have 'structure_locations' key."""
        data_dir = os.path.join(os.path.dirname(__file__), '..', 'data')
        file_path = os.path.join(data_dir, 'structure_locations.json')
        with open(file_path, 'r') as f:
            data = json.load(f)
    ...
    @pytest.mark.skip(reason="structure_locations.json may contain data from previous runs")
...
class TestRadiusSearchScaffold:
    ...
    def test_radius_search_has_required_attributes(self):
        ...
        searcher = RadiusSearch()
        assert hasattr(searcher, 'systems'), "Missing 'systems' attribute"
        assert hasattr(searcher, 'structure_locations'), "Missing 'structure_locations' attribute"
...
class TestStructureLocationsPath:
    ...
    @pytest.mark.skip(reason="structure_locations_path was never implemented as a module constant")
