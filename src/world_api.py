...
class WorldAPIClient:
    def __init__(self, base_url: str = None, cache_ttl: float = 30.0):
        # WORLD_API_BASE_URL env override takes precedence; otherwise use config-derived URL
        from src.config import load_config_from_env

        config = load_config_from_env()
        self.base_url = base_url or os.getenv("WORLD_API_BASE_URL") or config["world_api_url"]
    ...
    def _index_file(self) -> str:
        ...
        return get_data_path("system_index.json", env_specific=True)

    def _gate_graph_file(self) -> str:
        from src.config import get_data_path
        return get_data_path("gate_graph.json", env_specific=True)
