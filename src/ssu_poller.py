...
def _classify_ship(ship_name: str) -> str:
    """Classify a ship into a broad category for threat assessment."""
    if not ship_name:
        return "unknown"
    ship_lower = ship_name.lower()
    if "corvette" in ship_lower:
        return "corvette"
    elif "frigate" in ship_lower:
        return "frigate"
    elif "destroyer" in ship_lower:
        return "destroyer"
    elif "cruiser" in ship_lower:
        return "cruiser"
    elif "capital" in ship_lower or "carrier" in ship_lower or "dreadnought" in ship_lower:
        return "capital"
    else:
        return "other"
...
# blockchain_queries (nova_client) and structure_profile helpers are imported lazily at first use to
...
try:
    from src.structure_persistence import load_profile, save_profile
except Exception:  # pragma: no cover
...
async def poll_killmails(structure_id: str, system_id: int):
    """Poll killmails from blockchain first, fall back to World API."""
    from src.blockchain_killmails import get_killmails_from_blockchain
    ...
    kills = await get_killmails_from_blockchain(system_id)
    if kills is None:
        # Fall back to World API if blockchain unavailable
        try:
            kills = await world_api.get_killmails(system_id=system_id)
        except Exception as e:
            log.warning("Killmail poll failed (both blockchain and World API): %s", e)
            return
    ...
    for kill in (kills or []):
        ...
        # Extract timestamp for sorting/filtering
        ts_str = kill.get("time") or kill.get("timestamp") or ""

        # Extract attackers list
        attackers_list = kill.get("attackers") or []
        attacker_names = [a.get("name", "") for a in attackers_list if a.get("name")]
        primary_attacker = attacker_names[0] if attacker_names else None
        attacker_corp = attackers_list[0].get("corp") if attackers_list and attackers_list[0].get("corp") else None

        # Classify victim ship
        victim_ship = (
            kill.get("victimShip") or
            (kill.get("victim") or {}).get("ship", "Unknown")
        )
        ship_class = _classify_ship(victim_ship)
