...
class RadiusSearch:
    ...
    def __init__(self, systems_path: str = None, structure_locations_path: str = None):
        ...
        Args:
            ...
            structure_locations_path: Optional path to structure_locations.json.
                                     Defaults to data/{env}/structure_locations.json.
        """
        from src.config import get_data_path

        self.systems_path = systems_path or get_data_path("systems.json", env_specific=False)
        self.structure_locations_path = structure_locations_path or get_data_path("structure_locations.json", env_specific=True)
    ...
    def find_systems_within_radius(
        ...
        center_system_id = center_system.get("id")
        ...
        for system_id, system_data in self.systems.items():
            ...
            # Skip the center system itself
            if system_id == str(center_system_id):
                continue
    ...
    def cache_dir(self) -> Path:
        ...
        from src.config import get_data_path
        cache_path = get_data_path("cache", env_specific=False)
        return Path(cache_path)

    def get_cache_path(self, assembly_id: str) -> Path:
        """Get cache file path for assembly_id.

        Args:
            assembly_id: Structure assembly ID (slug)

        Returns:
            Path to cache file (creates parent directory if needed)
        """
        cache_dir = self.cache_dir / assembly_id
        cache_dir.mkdir(parents=True, exist_ok=True)
        return cache_dir / "radius_search.json"

    def load_cache(self, assembly_id: str) -> Optional[Dict[str, Any]]:
        ...
        Returns:
            Cache dict if found and valid, None otherwise
        """
        cache_file = self.get_cache_path(assembly_id)
        if not cache_file.exists():
            return None
        try:
            return json.loads(cache_file.read_text())
        except (json.JSONDecodeError, IOError) as e:
            log.warning(f"Failed to load cache for {assembly_id}: {e}")
            return None

    def save_cache(self, assembly_id: str, data: Dict[str, Any]) -> None:
        """Save cache to disk.
        ...
        try:
            ...
            log.debug(f"Saved radius search cache for {assembly_id}")
        except Exception as e:
            log.error(f"Failed to save cache for {assembly_id}: {e}")

    async def build_cache(
        self,
        assembly_id: str,
        center_system: str,
        radius_ly: float = 100.0
    ) -> Dict[str, Any]:
        """
        Build radius search cache for a structure.

        Steps:
        1. Find systems within radius using find_systems_within_radius()
        2. Query killmails for each system using get_killmails_for_system()
        3. Merge data and save cache to disk
        4. Return cache data

        Args:
            assembly_id: Structure assembly ID (slug)
            center_system: Center system name for radius search
            radius_ly: Search radius in light-years (default 100)

        Returns:
            Cache dict with structure, systems, and metadata
        """
        # Find systems within radius
        systems_in_radius = self.find_systems_within_radius(center_system, radius_ly)

        # Get kills for each system
        systems = []
        for system in systems_in_radius:
            system_id = system.get("id")
            name = system.get("name")

            # Get recent kills
            killmails = await self.get_killmails_for_system(system_id, hours=24) if system_id else []
            kills_24h = len(killmails)

            system_data = {
                "name": name,
                "distance_ly": round(system.get("distance_ly", 0), 2),
                "planets": self.count_planets(system),
                "safe_jump_temp": system.get("safe_jump_temp", 0),
                "kills_24h": kills_24h,
                "gates": system.get("gates", False)
            }
        ...
        cache_data = {
            ...
            "radius_ly": radius_ly,
            "static_data_loaded_at": now.isoformat() + "Z",
            "kills_last_updated": now.isoformat() + "Z",
            "systems": systems
        }

        # Save to disk
        self.save_cache(assembly_id, cache_data)

        return cache_data

    async def refresh_cache_kills(self, assembly_id: str) -> Dict[str, Any]:
        """
        Refresh kills data for existing cache without rebuilding static data.

        Re-queries killmails for each system in the cache and updates kill counts.
        Updates kills_last_updated timestamp but preserves static_data_loaded_at.
        ...
        Raises:
            ValueError: If cache not found for assembly_id
        """
        # Load existing cache
        cache = self.load_cache(assembly_id)
        if cache is None:
            raise ValueError(f"No cache found for assembly_id {assembly_id}")

        # Re-query kills for each system
        for system in cache["systems"]:
            system_name = system["name"]
            system_obj = self.get_system(system_name)
            if system_obj:
                system_id = system_obj.get("id")
                if system_id:
                    killmails = await self.get_killmails_for_system(system_id, hours=24)
                    system["kills_24h"] = len(killmails)

        # Update timestamp
        cache["kills_last_updated"] = datetime.now(timezone.utc).isoformat() + "Z"

        # Save updated cache
        self.save_cache(assembly_id, cache)
